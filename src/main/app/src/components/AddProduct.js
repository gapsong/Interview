...
import { saveProduct } from '../data/modules/products';
...
class AddProduct extends React.Component<Props> {
  ...
  addProduct(category : string, content : string) : void {
    this.props.dispatch(saveProduct(category, content));
  }
  ...
  onSubmit(e) {
    ...
    if (category && content) {
      this.addProduct(category.value.trim(), content.value.trim());
      category.value = '';
      content.value = '';
    }
...
export default withRouter(connect()(AddProduct));
